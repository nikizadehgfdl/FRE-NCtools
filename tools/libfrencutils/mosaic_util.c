...
double poly_area(const double x[], const double y[], int n)
{
  ...
  for (i=0;i<n;i++) {
    ...
    if(dx > M_PI)  dx = dx - 2.0*M_PI;
    if(dx < -M_PI) dx = dx + 2.0*M_PI;
    //if(dx <= -M_PI) dx = dx + 2.0*M_PI;//This is needed for fix_lon to work correctly 
    if (dx==0.0) continue;
...
int fix_lon0(double x[], double y[], int n, double tlon)
{
  double x_sum, dx;
  int i, nn = n, pole = 0;
...
{
  double x_sum, dx,help;
  int i,j,min_j, nn = n, pole = 0, near_south_pole = 0;

  for (i=0;i<nn;i++) if (fabs(y[i]+HPI)< 0.0093) near_south_pole = 1;
  if(near_south_pole){
    for(i=0;i<nn;i++) {
      nn=dsort(x,nn); //sort x's in ascending order
      if(x[nn-1]-x[0] >  M_PI) {
        x[nn-1]=x[nn-1]-TPI;
      }
      else {
...
int dsort(double x[], int n)
{
  ...
  for (i=0;i<nn;i++) {
    min_j = i;   // Assume elem i (x[i]) is the minimum
    for (j=i+1;j<nn;j++) if(x[j]<x[min_j]) min_j=j;
    // Swap x[i] and x[min_j]
    help=x[i];
    x[i]=x[min_j];
    x[min_j]=help;
  }

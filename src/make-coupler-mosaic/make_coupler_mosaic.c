 ...
 * should be even to avoid tiling error. I will come back to solve this issue
 * in the future.
 *
 */
/* Notes on the Algorithm
        ...
        *** Extending area_ocn[j=0,:]=area_ocn[j=1,:] is inaccurate if it's
   used Read OCN depth and set ocean mask omask=1 where wet
...
char *usage[] = {
  ...
  "[--wave_mosaic]",
  "              [--sea_level #]  [--interp_order #] [--mosaic_name "
  "mosaic_name]           ",
  "              [--area_ratio_thresh #] [--check ] [--verbose] "
  "[--print_memory]            ",
  " ",
  ...
  "might be ",
  "multiple tiles in a solo mosaic. The exchange grid information is between "
  "model   ",
  ...
  "Besides generating the exchange grid files, make_coupler_mosaic also "
  "generates the ",
  "coupler mosaic file (the file name will be mosaic_name.nc) which contains "
  "the atmos, ",
  "land and ocean mosaic path, ocean mosaic topog path and exchange grid file "
  "path. ",
  ...
  "required to be ",
  "                               logically rectangular grid. The file name "
  ...
  "                               ntiles = 1, The topography data is positive "
  "down.",
  " ",
  "OPTIONAL FLAGS",
  "",
  ...
  "                               information for each tile. Each grid is "
  "required to be ",
  "                               logically rectangular grid. When "
  "land_mosaic is not specified,",
  "                               atmosphere mosaic will be used to specify "
  "land mosaic.",
  "                               The file name can not be 'mosaic.nc'.",
  " ",
  "--wave_mosaic wave_mosaic.nc   specify the wave mosaic information. This "
  "file",
  "                               contains list of tile files which specify "
  "the grid ",
  "                               information for each tile. Each grid is "
  "required to be ",
  "                               logically rectangular grid. When "
  "wave_mosaic is specified, ",
  "                               exchange grid information between wave "
  "mosaic and ocean ",
  "                               mosaic will be generated. Otherwise none of "
  ...
  "value will be used",
  "                               to determine land/sea mask. When topography "
  "of  ",
  ...
  "                                                                           "
  "             ",
  "--area_ratio_thresh #          Criteria to decide if an overlap between "
  "any two model is an ",
  "                               exchange grid or not. When overlap "
  "area/model grid area is greater ",
  "                               than area_ratio_thresh, that overlap is "
  "saved as exchange grid. ",
  ...
  "            ",
  " ",
  "--rotate_poly                  Set to calculate polar polygon areas by "
  "caculating the area of a copy ",
  "                               of the polygon, with the copy being rotated "
  "far away from the pole. ",
  " ",
  "--verbose                      Set --verbose to print out messages during "
  "running.        ",
  "",
...
void
get_file_dir_and_name (char *file, char *filedir, char *filename)
{
  char *fptr = NULL;
  int siz;
...
void
get_global_grid (const char *grid_file, int nx, int ny, int x_refine,
                 int y_refine, double *x, double *y)
{
  ...
  for (j = 0; j <= nyc; j++)
    for (i = 0; i <= nxc; i++)
      x_local[j * (nxc + 1) + i]
          = tmp[(j * y_refine) * (nxc2 + 1) + i * x_refine];
                              ...
                              char wxo_file[MAXXGRIDFILE][STRING])
{
  ...
  print_provenance_gv_gca (fid, history, grid_version,
  ...
  if (nfile_wxo > 0)
    dim_wxo = mpp_def_dim (fid, "nfile_wXo", nfile_wxo);
  ...
  if (wmosaic)
    {
      id_wmosaic_dir
          = mpp_def_var (fid, "wav_mosaic_dir", MPP_CHAR, 1, &dim_string, 1,
                         "standard_name", "directory_storing_wave_mosaic");
      id_wmosaic_file
          = mpp_def_var (fid, "wav_mosaic_file", MPP_CHAR, 1, &dim_string, 1,
                         "standard_name", "wave_mosaic_file_name");
  ...
  if (nfile_axo > 0)
    {
      ...
      dims[1] = dim_string;
      id_axo_file
          = mpp_def_var (fid, "aXo_file", MPP_CHAR, 2, dims, 1,
  ...
  if (nfile_axl > 0)
    {
      ...
      dims[1] = dim_string;
      id_axl_file
          = mpp_def_var (fid, "aXl_file", MPP_CHAR, 2, dims, 1,
  ...
  if (nfile_lxo > 0)
    {
      ...
      dims[1] = dim_string;
      id_lxo_file
          = mpp_def_var (fid, "lXo_file", MPP_CHAR, 2, dims, 1,
  ...
  if (nfile_wxo > 0)
    {
      ...
      dims[1] = dim_string;
      id_wxo_file
          = mpp_def_var (fid, "wXo_file", MPP_CHAR, 2, dims, 1,
                         "standard_name", "wavXocn_exchange_grid_file");
    }
  mpp_end_def (fid);
  for (i = 0; i < 4; i++)
    {
  ...
  mpp_put_var_value_block (fid, id_amosaic_file, start, nwrite,
                           amosaic_file);
  ...
  mpp_put_var_value_block (fid, id_lmosaic_file, start, nwrite,
                           lmosaic_file);
  ...
  mpp_put_var_value_block (fid, id_omosaic_file, start, nwrite,
                           omosaic_file);
  ...
  if (wmosaic)
    {
      ...
      mpp_put_var_value_block (fid, id_wmosaic_dir, start, nwrite,
                               wmosaic_dir);
      nwrite[0] = strlen (wmosaic_file);
      mpp_put_var_value_block (fid, id_wmosaic_file, start, nwrite,
                               wmosaic_file);
...
void
get_global_data (const char *data_file, const char *fieldname, int nx, int ny,
                 double *data, int ioff, int joff)
{
  double *data_local;
  size_t start[4], nread[4];
  int nxc, nyc, isc, iec, jsc, jec;
  ...
  nyc = jec - jsc + 1;
  data_local
      = (double *)malloc ((nxc + ioff) * (nyc + joff) * sizeof (double));
  start[0] = jsc;
  start[1] = isc;
  start[2] = 0;
  start[3] = 0;
...
void
get_grid_global_area (int nx, int ny, const double *x, const double *y,
                      double *area)
{
...
int
get_nest_contact (const int *nx, const int *ny, int ncontacts,
                  const int *tile1, const int *tile2, const int *istart1,
                  const int *iend1, const int *jstart1, const int *jend1,
                  const int *istart2, const int *iend2, const int *jstart2,
                  const int *jend2, int *tile_nest, int *tile_parent,
                  int *is_nest, int *ie_nest, int *js_nest, int *je_nest,
                  int *is_parent, int *ie_parent, int *js_parent,
                  int *je_parent)
{
  ...
  for (n = 0; n < ncontacts; n++)
    {
      ...
      t1 = tile1[n] - 1;
      t2 = tile2[n] - 1;
      /* For nesting, the contact index of one tile must match its global
       * domain */
      if ((nx[t1] != nx1_contact || ny[t1] != ny1_contact)
          && (nx[t2] != nx2_contact || ny[t2] != ny2_contact))
        continue;
      if (nx1_contact == nx2_contact && ny1_contact == ny2_contact)
        mpp_error ("make_coupler_mosaic(get_nest_contact): There is no "
                   "refinement for the overlapping region");
      nnest++;
      if (nnest > 1)
        mpp_error ("make_coupler_mosaic(get_nest_contact): only support one "
                   "nest region, contact developer");
      if (nx2_contact * ny2_contact > nx1_contact * ny1_contact)
        {
          if (nx2_contact % nx1_contact || ny2_contact % ny1_contact)
            if (mpp_pe () == mpp_root_pe ())
              mpp_error ("make_coupler_mosaic(get_nest_contact):it is not a "
                         "integer refinement");
          is_nest[0] = istart2[n];
          ie_nest[0] = iend2[n];
          js_nest[0] = jstart2[n];
...
/* Extracted helper: read atmosphere mosaic and compute grid areas/contacts */
static void read_atmos_mosaic_block(const char *amosaic, char *amosaic_name,
                                   int x_refine, int y_refine,
                                   int interp_order, unsigned int check,
                                   int *ntile_atm, int **nxa, int **nya,
                                   double ***xatm, double ***yatm,
                                   double ***area_atm, double ***atm_xarea,
                                   char ***atile_name,
                                   int *atm_great_circle_algorithm,
                                   int *clip_method,
                                   double ***cart_xatm, double ***cart_yatm,
                                   double ***cart_zatm,
                                   int *is_nest, int *ie_nest, int *js_nest,
                                   int *je_nest, int *is_parent, int *ie_parent,
                                   int *js_parent, int *je_parent,
                                   int *tile_nest, int *tile_parent)
{
  int n, m_fid, g_fid, vid, gid, tid;
  int ncontacts, nnest;
  int *tile1 = NULL, *istart1 = NULL, *iend1 = NULL, *jstart1 = NULL,
      *jend1 = NULL;
  int *tile2 = NULL, *istart2 = NULL, *iend2 = NULL, *jstart2 = NULL,
      *jend2 = NULL;
  size_t start[4], nread[4];
  ...
  *area_atm = (double **)malloc ((*ntile_atm) * sizeof (double *));
  if (check)
    *atm_xarea = (double **)malloc ((*ntile_atm) * sizeof (double *));
  ...
  for (n = 0; n < *ntile_atm; n++)
    {
      ...
      mpp_close (g_fid);
      if ((*nxa)[n] % x_refine != 0)
        mpp_error ("make_coupler_mosaic: atmos supergrid x-size can not be divided by x_refine");
      if ((*nya)[n] % y_refine != 0)
        mpp_error ("make_coupler_mosaic: atmos supergrid y-size can not be divided by y_refine");
      (*nxa)[n] /= x_refine;
      (*nya)[n] /= y_refine;
  ...
  if (ncontacts > 0)
    {
      ...
      nnest = get_nest_contact ((*nxa), (*nya), ncontacts, tile1, tile2, istart1, iend1, jstart1, jend1, istart2, iend2, jstart2, jend2, tile_nest, tile_parent, is_nest, ie_nest, js_nest, je_nest, is_parent, ie_parent, js_parent, je_parent);
      free (tile1);
      free (tile2);
      free (istart1);
      free (iend1);
...
/* Extracted helper: read land mosaic and compute grid areas */
    static void read_land_mosaic_block(const char *lmosaic, char *lmosaic_name,
                                      int x_refine, int y_refine,
                                      int clip_method,
                                      int *ntile_lnd, int **nxl, int **nyl,
                                      double ***xlnd, double ***ylnd,
                                      double ***area_lnd, char ***ltile_name,
                                      int *lnd_great_circle_algorithm,
                                      double ***cart_xlnd, double ***cart_ylnd,
                                      double ***cart_zlnd)
    {
      int n, m_fid, g_fid, vid, gid, tid;
      size_t start[4], nread[4];
      ...
      for (n = 0; n < *ntile_lnd; n++)
        {
          ...
          mpp_close (g_fid);
          if ((*nxl)[n] % x_refine != 0)
            mpp_error ("make_coupler_mosaic: land supergrid x-size can not be divided by x_refine");
          if ((*nyl)[n] % y_refine != 0)
            mpp_error ("make_coupler_mosaic: land supergrid y-size can not be divided by y_refine");
          (*nxl)[n] /= x_refine;
          (*nyl)[n] /= y_refine;
          ...
          /* scale grid from degree to radian */
    ...
    /* Extracted helper: read ocean mosaic boundaries and compute grid areas */
    static void read_ocean_mosaic_block(const char *omosaic, char *omosaic_name,
                                       int x_refine, int y_refine,
                                       int *ntile_ocn, int **nxo, int **nyo,
                                       double ***xocn, double ***yocn,
                                       double ***area_ocn, char ***otile_name,
                                       int *ocn_south_ext,
                                       int *ocn_great_circle_algorithm,
                                       int clip_method,
                                       double ***cart_xocn, double ***cart_yocn,
                                       double ***cart_zocn,
                                       const char *otopog, double ***omask,
                                       double sea_level, unsigned int verbose)
    {
      ...
      *otile_name = (char **)malloc ((*ntile_ocn) * sizeof (char *));
      ...
      /* For the purpose of reproducing between processor count, the layout is set to (1, npes). */
      ...
      for (n = 0; n < *ntile_ocn; n++)
        {
          ...
          if ((*nxo)[n] % x_refine != 0)
            mpp_error ("make_coupler_mosaic: ocean supergrid x-size can not be divided by x_refine");
          if ((*nyo)[n] % y_refine != 0)
            mpp_error ("make_coupler_mosaic: ocean supergrid y-size can not be divided by y_refine");
          (*nxo)[n] /= x_refine;
          (*nyo)[n] /= y_refine;
          ...
          /* sometimes the ocean grid only covers part of the globe... extend south if needed */
          if (*ntile_ocn == 1)
            {
              int na;
              int is_uniform;
              ...
              if (!is_uniform && mpp_pe () == mpp_root_pe ())
              ...
              min_atm_lat = -90. * D2R;
              if (tmpy[0] * D2R > min_atm_lat + TINY_VALUE)
                { /* extend one point in south direction*/
                  *ocn_south_ext = 1;
                  if (mpp_pe () == mpp_root_pe ())
                    printf ("make_coupler_mosaic: one row is added to the south end to cover the globe\n");
                }
            }
          nyo_old = (*nyo)[n];
          (*nyo)[n] += *ocn_south_ext;
      ...
      if (clip_method == GREAT_CIRCLE_CLIP)
        {
          *cart_xocn = (double **)malloc ((*ntile_ocn) * sizeof (double *));
          *cart_yocn = (double **)malloc ((*ntile_ocn) * sizeof (double *));
        ...
        {
          if (mpp_pe () == mpp_root_pe ())
            printf ("make_coupler_mosaic: Calculating area_ocn based on lat-lon segments between adjacent grid points.\n");
          for (n = 0; n < *ntile_ocn; n++)
            {
      ...
      for (n = 0; n < *ntile_ocn; n++)
        {
          ...
          if (n == 0)
            {
              ...
              if (*ntile_ocn != ntiles)
                mpp_error ("make_coupler_mosaic: dimlen ntiles in mosaic file is not the same as dimlen in topog file");
            }
          ...
          if (nx != (*nxo)[n] || ny + *ocn_south_ext != (*nyo)[n])
            mpp_error ("make_coupler_mosaic: grid size mismatch between mosaic file and topog file");
          if (ntiles == 1)
            {
          ...
          if (mask_name_exist)
            {
              if (mpp_pe () == mpp_root_pe () && verbose)
                printf ("\nNOTE from make_coupler_mosaic: the ocean land/sea mask will be determined by field area_frac from file %s\n", otopog);
    ...
    /* Extracted helper: read wave mosaic and compute grid areas */
    static void read_wave_mosaic_block(const char *wmosaic, const char *omosaic,
                                      char *wmosaic_name, int x_refine, int y_refine,
                                      int *ntile_wav, int **nxw, int **nyw,
                                      double ***xwav, double ***ywav,
                                      double ***area_wav, char ***wtile_name,
                                      int *wav_same_as_ocn,
                                      int clip_method,
                                      double ***cart_xwav, double ***cart_ywav,
                                      double ***cart_zwav)
      ...
      for (n = 0; n < *ntile_wav; n++)
        {
          ...
          mpp_close (g_fid);
          if ((*nxw)[n] % x_refine != 0)
            mpp_error ("make_coupler_mosaic: wave supergrid x-size can not be divided by x_refine");
          if ((*nyw)[n] % y_refine != 0)
            mpp_error ("make_coupler_mosaic: wave supergrid y-size can not be divided by y_refine");
          (*nxw)[n] /= x_refine;
          (*nyw)[n] /= y_refine;
...
/* Extracted helper: aggregate exchange areas and write land/ocean masks */
static void finalize_and_write_masks(int ntile_atm, int ntile_lnd, int ntile_ocn,
                                    int *nxa, int *nya, int *nxl, int *nyl,
                                    int *nxo, int *nyo, size_t **naxl,
                                    size_t **naxo,
                                    int ***atmxlnd_ia, int ***atmxlnd_ja,
                                    int ***atmxlnd_il, int ***atmxlnd_jl,
                                    int ***atmxocn_ia, int ***atmxocn_ja,
                                    int ***atmxocn_io, int ***atmxocn_jo,
                                    double ***atmxlnd_area,
                                    double ***atmxocn_area,
                                    double ***atmxlnd_clon,
                                    double ***atmxlnd_clat,
                                    ...
                                    double ***atmxocn_dia,
                                    double ***atmxocn_dja,
                                    double **area_lnd, double **area_ocn,
                                    double **area_atm, double **omask,
                                    int ocn_south_ext, int interp_order,
                                    int tile_nest, char *history,
                                    char *grid_version, int clip_method,
                                    unsigned int verbose, int *nbad)
{
                  ...
                  if (nxgrid > 0)
                    {
                      ...
                      mpp_gather_field_int (naxl[na][nl],
                                            atmxlnd_il[na][nl], g_il);
                      mpp_gather_field_int (naxl[na][nl],
                                            atmxlnd_jl[na][nl], g_jl);
                      mpp_gather_field_double (
                          naxl[na][nl], atmxlnd_area[na][nl], g_area);
                      for (i = 0; i < nxgrid; i++)
                        {
                  ...
                  if (nxgrid > 0)
                    {
                      ...
                      g_area = (double *)malloc (nxgrid * sizeof (double));
                      mpp_gather_field_int (naxo[na][no],
                                            atmxocn_io[na][no], g_io);
                      mpp_gather_field_int (naxo[na][no],
                                            atmxocn_jo[na][no], g_jo);
                      mpp_gather_field_double (
                          naxo[na][no], atmxocn_area[na][no], g_area);
                      for (i = 0; i < nxgrid; i++)
                        {
                  ...
                  if (nxgrid > 0)
                    {
                      ...
                      mpp_gather_field_int (naxl[na][nl],
                                            atmxlnd_ia[na][nl], g_ia);
                      mpp_gather_field_int (naxl[na][nl],
                                            atmxlnd_ja[na][nl], g_ja);
                      mpp_gather_field_int (naxl[na][nl],
                                            atmxlnd_il[na][nl], g_il);
                      mpp_gather_field_int (naxl[na][nl],
                                            atmxlnd_jl[na][nl], g_jl);
                      mpp_gather_field_double (
                          naxl[na][nl], atmxlnd_area[na][nl], g_area);
                      mpp_gather_field_double (
                          naxl[na][nl], atmxlnd_clon[na][nl], g_clon);
                      mpp_gather_field_double (
                          naxl[na][nl], atmxlnd_clat[na][nl], g_clat);
                      for (i = 0; i < nxgrid; i++)
                        {
                  ...
                  if (nxgrid > 0)
                    {
                      ...
                      g_clat = (double *)malloc (nxgrid * sizeof (double));
                      mpp_gather_field_int (naxo[na][no],
                                            atmxocn_ia[na][no], g_ia);
                      mpp_gather_field_int (naxo[na][no],
                                            atmxocn_ja[na][no], g_ja);
                      mpp_gather_field_int (naxo[na][no],
                                            atmxocn_io[na][no], g_io);
                      mpp_gather_field_int (naxo[na][no],
                                            atmxocn_jo[na][no], g_jo);
                      mpp_gather_field_double (
                          naxo[na][no], atmxocn_area[na][no], g_area);
                      mpp_gather_field_double (
                          naxo[na][no], atmxocn_clon[na][no], g_clon);
                      mpp_gather_field_double (
                          naxo[na][no], atmxocn_clat[na][no], g_clat);
                      for (i = 0; i < nxgrid; i++)
                        {
                          ii = g_ja[i] * nxa[na] + g_ia[i];
                          a_area[ii] += g_area[i];
  ...
  /* calculate ocean_frac and compare ocean_frac with omask and write out ocn_frac */
  if (mpp_pe () == mpp_root_pe ())
    {
      int io, jo;
      double ocn_frac;
      ...
      for (no = 0; no < ntile_ocn; no++)
        {
          ...
          id_mask = mpp_def_var (
              fid, "mask", MPP_DOUBLE, 2, dims, 2, "standard_name",
              "ocean fraction at T-cell centers", "units", "none");
          id_areao = mpp_def_var (fid, "areaO", MPP_DOUBLE, 2, dims, 2,
                                  "standard_name", "ocean grid area",
                                  "units", "none");
          id_areax = mpp_def_var (
              fid, "areaX", MPP_DOUBLE, 2, dims, 2, "standard_name",
    ...
    for (nl = 0; nl < ntile_lnd; nl++)
      {
        ...
        id_a_a = mpp_def_var (fid, "area_atm", MPP_DOUBLE, 2, dims, 2,
                               "standard_name", "area atm ", "units", "none");
        id_a_l = mpp_def_var (fid, "area_lnd", MPP_DOUBLE, 2, dims, 2,
                               "standard_name", "area land ", "units", "none");
        id_l_a = mpp_def_var (fid, "l_area", MPP_DOUBLE, 2, dims, 2,
                              "standard_name", "land x area", "units",
                              "none");
        mpp_end_def (fid);
        mpp_put_var_value (fid, id_mask, mask);
  ...
  free (l_area);
    ...
    static void compute_atm_lnd_ocn_exchange(int ntile_atm, int ntile_lnd, int ntile_ocn,
                                             ...
                                             double **yatm, double **area_atm,
                                             double **atm_xarea,
                                             int *nxl, int *nyl, double **xlnd,
                                             double **ylnd, double **area_lnd,
                                             int *nxo, int *nyo, double **xocn,
                                             double **yocn, double **area_ocn,
                                             double **omask, int interp_order,
                                             int clip_method,
                                             double area_ratio_thresh,
                                             unsigned int verbose,
                                             unsigned int lnd_same_as_atm, unsigned int ocn_same_as_atm,
                                             unsigned int print_memory,
                                             size_t **naxl, size_t **naxo,
                                             int ***atmxlnd_ia, int ***atmxlnd_ja,
                                             int ***atmxlnd_il, int ***atmxlnd_jl,
                                             int ***atmxocn_ia, int ***atmxocn_ja,
                                             int ***atmxocn_io, int ***atmxocn_jo,
                                             ...
                                             double ***atmxocn_clat,
                                             double **cart_xatm, double **cart_yatm,
                                             double **cart_zatm, double **cart_xlnd,
                                             double **cart_ylnd, double **cart_zlnd,
                                             double **cart_xocn, double **cart_yocn,
                                             double **cart_zocn)
    {
      int na, nl, no, n, la, ia, ja, il, jl, io, jo, is, ie, js, je;
      int k, l;
      ...
      domain2D Dom;
      double yy, min_area;
      int *js_lnd, *je_lnd, *js_ocn, *je_ocn, *is_lnd, *ie_lnd, *is_ocn, *ie_ocn;
      char mesg[256];
      ...
      for (na = 0; na < ntile_atm; na++)
        {
              ...
              if (clip_method == GREAT_CIRCLE_CLIP)
                {
                  ...
                  n2 = (ja + 1) * (nxa[na] + 1) + ia + 1;
                  n3 = ja * (nxa[na] + 1) + ia + 1;
                  xa[0] = cart_xatm[na][n0];
                  ya[0] = cart_yatm[na][n0];
                  za[0] = cart_zatm[na][n0];
                  xa[1] = cart_xatm[na][n1];
                  ya[1] = cart_yatm[na][n1];
                  za[1] = cart_zatm[na][n1];
                  xa[2] = cart_xatm[na][n2];
                  ya[2] = cart_yatm[na][n2];
                  za[2] = cart_zatm[na][n2];
                  xa[3] = cart_xatm[na][n3];
                  ya[3] = cart_yatm[na][n3];
                  za[3] = cart_zatm[na][n3];
                }
              else
                {
                  n0 = ja * (nxa[na] + 1) + ia;
                        ...
                        if (clip_method == GREAT_CIRCLE_CLIP)
                          {
                            ...
                            n2 = (jl + 1) * (nxl[nl] + 1) + il + 1;
                            n3 = jl * (nxl[nl] + 1) + il + 1;
                            xl[0] = cart_xlnd[nl][n0];
                            yl[0] = cart_ylnd[nl][n0];
                            zl[0] = cart_zlnd[nl][n0];
                            xl[1] = cart_xlnd[nl][n1];
                            yl[1] = cart_ylnd[nl][n1];
                            zl[1] = cart_zlnd[nl][n1];
                            xl[2] = cart_xlnd[nl][n2];
                            yl[2] = cart_ylnd[nl][n2];
                            zl[2] = cart_zlnd[nl][n2];
                            xl[3] = cart_xlnd[nl][n3];
                            yl[3] = cart_ylnd[nl][n3];
                            zl[3] = cart_zlnd[nl][n3];
                            if (lnd_same_as_atm)
                              {
                            ...
                            else
                              {
                                n_out = clip_2dx2d_great_circle (
                                    xa, ya, za, 4, xl, yl, zl, 4, x_out, y_out,
                                    z_out);
                              }
                          }
                        else
                                ...
                                if (na == nl && ja == jl && ia == il)
                                  {
                                    if (na_in != nl_in)
                                      {
                                        printf ("Error: LND and AM grids are the same but na_in != nl_in, n_out,%d,%d,%d",
                                                na_in, nl_in, n_out);
                                        mpp_error (
                                            "make_coupler_mosaic: inconsistent number of grid box coreners");
                                      }
                                    n_out = nl_in;
                                    for (n = 0; n < n_out; n++)
                                      {
                            ...
                            if (xarea / min_area > area_ratio_thresh)
                              {
                                ...
                                ++count;
                                if (count > MX)
                                  mpp_error ("make_coupler_mosaic: count is greater than MX, increase MX");
                              }
                          }
                      }
                    ...
                    for (io = is_ocn[no]; io <= ie_ocn[no]; io++)
                      {
                        ...
                        if (clip_method == GREAT_CIRCLE_CLIP)
                          {
                            ...
                            n2 = (jo + 1) * (nxo[no] + 1) + io + 1;
                            n3 = jo * (nxo[no] + 1) + io + 1;
                            xo[0] = cart_xocn[no][n0];
                            yo[0] = cart_yocn[no][n0];
                            zo[0] = cart_zocn[no][n0];
                            xo[1] = cart_xocn[no][n1];
                            yo[1] = cart_yocn[no][n1];
                            zo[1] = cart_zocn[no][n1];
                            xo[2] = cart_xocn[no][n2];
                            yo[2] = cart_yocn[no][n2];
                            zo[2] = cart_zocn[no][n2];
                            xo[3] = cart_xocn[no][n3];
                            yo[3] = cart_yocn[no][n3];
                            zo[3] = cart_zocn[no][n3];
                          }
                        else
                          {
                            n0 = jo * (nxo[no] + 1) + io;
                        ...
                        if (ocn_frac > MIN_AREA_FRAC)
                          {
                            if (clip_method == GREAT_CIRCLE_CLIP)
                              {
                                n_out = clip_2dx2d_great_circle (
                                    xa, ya, za, 4, xo, yo, zo, 4, x_out, y_out,
                                    z_out);
                              }
                            else
                              {
                                ...
                                if (xarea / min_area > area_ratio_thresh)
                                  {
                                    ...
                                    if (naxo[na][no] > get_maxxgrid())
                                      {
                                        char msg[200];
                                        snprintf(msg, sizeof(msg), "naxo exceeds MAXXGRID limit (%d), increase the value using --maxxgrid flag", naxo[na][no]);
                                        mpp_error(msg);
                                      }
                                  }
                              }
                                ...
                                */
                                ...
                                if (clip_method == GREAT_CIRCLE_CLIP)
                                  n_out = clip_2dx2d_great_circle (
                                      ...
                                      num_v[l], xo, yo, zo, 4, x_out, y_out, z_out);
                                else
                                  {
                  ...
                  if (axl_area[l] / min_area > area_ratio_thresh)
                    {
                      ...
                      if (naxl[na][nl] > get_maxxgrid())
                        {
                          char msg[200];
                          snprintf(msg, sizeof(msg), "naxl (%d) exceeds MAXXGRID limit, increase the value using --maxxgrid flag", naxl[na][nl]);
                          mpp_error(msg);
                        }
                    }
                }
          ...
          if (mpp_pe () == mpp_root_pe () && verbose)
            printf ("Number of atmxlnd cells for ATM tile %d is %i .\n", na + 1, atmxlnd_count);
...
static void allocate_atm_exchange_arrays(int ntile_atm, int ntile_lnd,
                                         int ntile_ocn, int interp_order,
                                         size_t ***naxl_ptr,
                                         size_t ***naxo_ptr,
                                         double ****atmxlnd_area_ptr,
                                         int ****atmxlnd_ia_ptr,
                                         int ****atmxlnd_ja_ptr,
                                         int ****atmxlnd_il_ptr,
                                         int ****atmxlnd_jl_ptr,
                                         double ****atmxocn_area_ptr,
                                         int ****atmxocn_ia_ptr,
                                         int ****atmxocn_ja_ptr,
                                         int ****atmxocn_io_ptr,
                                         int ****atmxocn_jo_ptr,
                                         double ****atmxlnd_dia_ptr,
                                         double ****atmxlnd_dja_ptr,
                                         double ****atmxlnd_dil_ptr,
                                         double ****atmxlnd_djl_ptr,
                                         double ****atmxocn_dia_ptr,
                                         double ****atmxocn_dja_ptr,
                                         double ****atmxocn_dio_ptr,
                                         double ****atmxocn_djo_ptr,
                                         double ****atmxlnd_clon_ptr,
                                         double ****atmxlnd_clat_ptr,
                                         double ****atmxocn_clon_ptr,
                                         double ****atmxocn_clat_ptr)
{
  int na, nl, no;

  *naxl_ptr = (size_t **)malloc (ntile_atm * sizeof (size_t *));
  *naxo_ptr = (size_t **)malloc (ntile_atm * sizeof (size_t *));
  *atmxlnd_area_ptr = (double ***)malloc (ntile_atm * sizeof (double **));
  *atmxlnd_ia_ptr = (int ***)malloc (ntile_atm * sizeof (int **));
  *atmxlnd_ja_ptr = (int ***)malloc (ntile_atm * sizeof (int **));
  *atmxlnd_il_ptr = (int ***)malloc (ntile_atm * sizeof (int **));
  *atmxlnd_jl_ptr = (int ***)malloc (ntile_atm * sizeof (int **));
  *atmxocn_area_ptr = (double ***)malloc (ntile_atm * sizeof (double **));
  *atmxocn_ia_ptr = (int ***)malloc (ntile_atm * sizeof (int **));
  *atmxocn_ja_ptr = (int ***)malloc (ntile_atm * sizeof (int **));
  *atmxocn_io_ptr = (int ***)malloc (ntile_atm * sizeof (int **));
  *atmxocn_jo_ptr = (int ***)malloc (ntile_atm * sizeof (int **));

  if (interp_order == 2)
    {
      *atmxlnd_dia_ptr = (double ***)malloc (ntile_atm * sizeof (double **));
      *atmxlnd_dja_ptr = (double ***)malloc (ntile_atm * sizeof (double **));
      *atmxlnd_dil_ptr = (double ***)malloc (ntile_atm * sizeof (double **));
      *atmxlnd_djl_ptr = (double ***)malloc (ntile_atm * sizeof (double **));
      *atmxocn_dia_ptr = (double ***)malloc (ntile_atm * sizeof (double **));
      *atmxocn_dja_ptr = (double ***)malloc (ntile_atm * sizeof (double **));
      *atmxocn_dio_ptr = (double ***)malloc (ntile_atm * sizeof (double **));
      *atmxocn_djo_ptr = (double ***)malloc (ntile_atm * sizeof (double **));
      *atmxlnd_clon_ptr = (double ***)malloc (ntile_atm * sizeof (double **));
      *atmxlnd_clat_ptr = (double ***)malloc (ntile_atm * sizeof (double **));
      *atmxocn_clon_ptr = (double ***)malloc (ntile_atm * sizeof (double **));
      *atmxocn_clat_ptr = (double ***)malloc (ntile_atm * sizeof (double **));
    }

  for (na = 0; na < ntile_atm; na++)
    {
      (*naxl_ptr)[na] = (size_t *)malloc (ntile_lnd * sizeof (size_t));
      (*naxo_ptr)[na] = (size_t *)malloc (ntile_ocn * sizeof (size_t));
      (*atmxlnd_area_ptr)[na] = (double **)malloc (ntile_lnd * sizeof (double *));
      (*atmxlnd_ia_ptr)[na] = (int **)malloc (ntile_lnd * sizeof (int *));
      (*atmxlnd_ja_ptr)[na] = (int **)malloc (ntile_lnd * sizeof (int *));
      (*atmxlnd_il_ptr)[na] = (int **)malloc (ntile_lnd * sizeof (int *));
      (*atmxlnd_jl_ptr)[na] = (int **)malloc (ntile_lnd * sizeof (int *));
      (*atmxocn_area_ptr)[na] = (double **)malloc (ntile_ocn * sizeof (double *));
      (*atmxocn_ia_ptr)[na] = (int **)malloc (ntile_ocn * sizeof (int *));
      (*atmxocn_ja_ptr)[na] = (int **)malloc (ntile_ocn * sizeof (int *));
      (*atmxocn_io_ptr)[na] = (int **)malloc (ntile_ocn * sizeof (int *));
      (*atmxocn_jo_ptr)[na] = (int **)malloc (ntile_ocn * sizeof (int *));

      if (interp_order == 2)
        {
          (*atmxlnd_dia_ptr)[na] = (double **)malloc (ntile_lnd * sizeof (double *));
          (*atmxlnd_dja_ptr)[na] = (double **)malloc (ntile_lnd * sizeof (double *));
          (*atmxlnd_dil_ptr)[na] = (double **)malloc (ntile_lnd * sizeof (double *));
          (*atmxlnd_djl_ptr)[na] = (double **)malloc (ntile_lnd * sizeof (double *));
          (*atmxocn_dia_ptr)[na] = (double **)malloc (ntile_ocn * sizeof (double *));
          (*atmxocn_dja_ptr)[na] = (double **)malloc (ntile_ocn * sizeof (double *));
          (*atmxocn_dio_ptr)[na] = (double **)malloc (ntile_ocn * sizeof (double *));
          (*atmxocn_djo_ptr)[na] = (double **)malloc (ntile_ocn * sizeof (double *));
          (*atmxlnd_clon_ptr)[na] = (double **)malloc (ntile_lnd * sizeof (double *));
          (*atmxlnd_clat_ptr)[na] = (double **)malloc (ntile_lnd * sizeof (double *));
          (*atmxocn_clon_ptr)[na] = (double **)malloc (ntile_ocn * sizeof (double *));
          (*atmxocn_clat_ptr)[na] = (double **)malloc (ntile_ocn * sizeof (double *));
        }

      for (nl = 0; nl < ntile_lnd; nl++)
        {
          (*atmxlnd_area_ptr)[na][nl] = (double *)malloc (get_maxxgrid() * sizeof (double));
          (*atmxlnd_ia_ptr)[na][nl] = (int *)malloc (get_maxxgrid() * sizeof (int));
          (*atmxlnd_ja_ptr)[na][nl] = (int *)malloc (get_maxxgrid() * sizeof (int));
          (*atmxlnd_il_ptr)[na][nl] = (int *)malloc (get_maxxgrid() * sizeof (int));
          (*atmxlnd_jl_ptr)[na][nl] = (int *)malloc (get_maxxgrid() * sizeof (int));
          if (interp_order == 2)
            {
              (*atmxlnd_clon_ptr)[na][nl] = (double *)malloc (get_maxxgrid() * sizeof (double));
              (*atmxlnd_clat_ptr)[na][nl] = (double *)malloc (get_maxxgrid() * sizeof (double));
              (*atmxlnd_dia_ptr)[na][nl] = (double *)malloc (get_maxxgrid() * sizeof (double));
              (*atmxlnd_dja_ptr)[na][nl] = (double *)malloc (get_maxxgrid() * sizeof (double));
              (*atmxlnd_dil_ptr)[na][nl] = (double *)malloc (get_maxxgrid() * sizeof (double));
              (*atmxlnd_djl_ptr)[na][nl] = (double *)malloc (get_maxxgrid() * sizeof (double));
            }
        }

      for (no = 0; no < ntile_ocn; no++)
        {
          (*atmxocn_area_ptr)[na][no] = (double *)malloc (get_maxxgrid() * sizeof (double));
          (*atmxocn_ia_ptr)[na][no] = (int *)malloc (get_maxxgrid() * sizeof (int));
          (*atmxocn_ja_ptr)[na][no] = (int *)malloc (get_maxxgrid() * sizeof (int));
          (*atmxocn_io_ptr)[na][no] = (int *)malloc (get_maxxgrid() * sizeof (int));
          (*atmxocn_jo_ptr)[na][no] = (int *)malloc (get_maxxgrid() * sizeof (int));
          if (interp_order == 2)
            {
              (*atmxocn_clon_ptr)[na][no] = (double *)malloc (get_maxxgrid() * sizeof (double));
              (*atmxocn_clat_ptr)[na][no] = (double *)malloc (get_maxxgrid() * sizeof (double));
              (*atmxocn_dia_ptr)[na][no] = (double *)malloc (get_maxxgrid() * sizeof (double));
              (*atmxocn_dja_ptr)[na][no] = (double *)malloc (get_maxxgrid() * sizeof (double));
              (*atmxocn_dio_ptr)[na][no] = (double *)malloc (get_maxxgrid() * sizeof (double));
              (*atmxocn_djo_ptr)[na][no] = (double *)malloc (get_maxxgrid() * sizeof (double));
            }
        }
    }
}
...
static void write_atm_exchange_files(int ntile_atm, int ntile_lnd, int ntile_ocn,
                                     ...
                                     int ***atmxlnd_ia, int ***atmxlnd_ja,
                                     int ***atmxlnd_il, int ***atmxlnd_jl,
                                     double ***atmxlnd_dia, double ***atmxlnd_dja,
                                     double ***atmxlnd_dil, double ***atmxlnd_djl,
                                     double ***atmxlnd_area,
                                     int ***atmxocn_ia, int ***atmxocn_ja,
                                     int ***atmxocn_io, int ***atmxocn_jo,
                                     double ***atmxocn_dia, double ***atmxocn_dja,
                                     double ***atmxocn_dio, double ***atmxocn_djo,
                                     double ***atmxocn_area,
                                     char axl_file[MAXXGRIDFILE][STRING],
                                     char axo_file[MAXXGRIDFILE][STRING],
                                     char *amosaic_name, char **atile_name,
                                     char *lmosaic_name, char **ltile_name,
                                     char *omosaic_name, char **otile_name,
                                     char *history, char *grid_version,
                                     int clip_method, int interp_order,
                                     int same_mosaic, int check,
                                     double **atm_xarea,
                                     int *nfile_axl, int *nfile_axo,
                                     double *axl_area_sum, double *axl_area_sum_nest,
                                     double *axo_area_sum, double *axo_area_sum_nest,
                                     int ocn_south_ext)
{
  int na, nl, no, i, n;
  for (na = 0; na < ntile_atm; na++)
          ...
          if (nxgrid > 0)
            {
              ...
              if (same_mosaic)
                sprintf (axl_file[*nfile_axl], "atm_%s_%sXlnd_%s_%s.nc",
                         amosaic_name, atile_name[na], lmosaic_name,
                         ltile_name[nl]);
              else
                sprintf (axl_file[*nfile_axl], "%s_%sX%s_%s.nc", amosaic_name,
                         atile_name[na], lmosaic_name, ltile_name[nl]);
              sprintf (contact, "%s:%s::%s:%s", amosaic_name,
                       atile_name[na], lmosaic_name, ltile_name[nl]);
              ...
              if (interp_order == 2)
                {
                  id_contact = mpp_def_var (
                      fid, "contact", MPP_CHAR, 1, &dim_string, 7,
                      "standard_name", "grid_contact_spec", "contact_type",
                      "exchange", "parent1_cell", "tile1_cell",
                      "parent2_cell", "tile2_cell", "xgrid_area_field",
                      "xgrid_area", "distant_to_parent1_centroid",
                      "tile1_distance", "distant_to_parent2_centroid",
                      "tile2_distance");
                }
              else
                {
                  id_contact = mpp_def_var (
                      fid, "contact", MPP_CHAR, 1, &dim_string, 5,
                      "standard_name", "grid_contact_spec", "contact_type",
                      "exchange", "parent1_cell", "tile1_cell",
                      "parent2_cell", "tile2_cell", "xgrid_area_field",
                      "xgrid_area");
              ...
              dims[0] = dim_ncells;
              dims[1] = dim_two;
              id_tile1_cell = mpp_def_var (fid, "tile1_cell", MPP_INT, 2,
                                           dims, 1, "standard_name",
                                           "parent_cell_indices_in_mosaic1");
              id_tile2_cell = mpp_def_var (fid, "tile2_cell", MPP_INT, 2,
                                           dims, 1, "standard_name",
                                           "parent_cell_indices_in_mosaic2");
              id_xgrid_area = mpp_def_var (
                  fid, "xgrid_area", MPP_DOUBLE, 1, &dim_ncells, 2,
                  "standard_name", "exchange_grid_area", "units", "m2");
              if (interp_order == 2)
                {
                  id_tile1_dist
                      = mpp_def_var (fid, "tile1_distance", MPP_DOUBLE, 2,
                                     dims, 1, "standard_name",
                                     "distance_from_parent1_cell_centroid");
                  id_tile2_dist
                      = mpp_def_var (fid, "tile2_distance", MPP_DOUBLE, 2,
                                     dims, 1, "standard_name",
              ...
              mpp_put_var_value_block (fid, id_contact, start, nwrite,
                                       contact);
              nwrite[0] = nxgrid;
              ...
              if (check)
                {
                  ...
                  mpp_gather_field_int (naxl[na][nl], atmxlnd_ia[na][nl],
                                        gdata_ia);
                  mpp_gather_field_int (naxl[na][nl], atmxlnd_ja[na][nl],
                                        gdata_ja);
                  for (n = 0; n < nxgrid; n++)
                    {
                      ja = gdata_ja[n] - 1;
              ...
              mpp_gather_field_int (naxl[na][nl], atmxlnd_ia[na][nl],
                                    gdata_int);
              mpp_put_var_value_block (fid, id_tile1_cell, start, nwrite,
                                       gdata_int);
              mpp_gather_field_int (naxl[na][nl], atmxlnd_il[na][nl],
                                    gdata_int);
              mpp_put_var_value_block (fid, id_tile2_cell, start, nwrite,
                                       gdata_int);
              start[1] = 1;
              mpp_gather_field_int (naxl[na][nl], atmxlnd_ja[na][nl],
                                    gdata_int);
              mpp_put_var_value_block (fid, id_tile1_cell, start, nwrite,
                                       gdata_int);
              mpp_gather_field_int (naxl[na][nl], atmxlnd_jl[na][nl],
                                    gdata_int);
              mpp_put_var_value_block (fid, id_tile2_cell, start, nwrite,
                                       gdata_int);
              if (interp_order == 2)
          ...
          if (nxgrid > 0)
            {
              ...
              if (same_mosaic)
                sprintf (axo_file[*nfile_axo], "atm_%s_%sXocn_%s_%s.nc",
                         amosaic_name, atile_name[na], omosaic_name,
                         otile_name[no]);
              else
              ...
              for (i = 0; i < naxo[na][no]; i++)
                {
                  ...
                  ++(atmxocn_io[na][no][i]);
                  atmxocn_jo[na][no][i]
                      += 1 - ocn_south_ext; /* possible one artificial j-level
                                               is added at south end */
                }
              ...
              if (check)
                {
                  ...
                  gdata_ia = (int *)malloc (nxgrid * sizeof (int));
                  gdata_ja = (int *)malloc (nxgrid * sizeof (int));
                  mpp_gather_field_int (naxo[na][no], atmxocn_ia[na][no],
                                        gdata_ia);
                  mpp_gather_field_int (naxo[na][no], atmxocn_ja[na][no],
                                        gdata_ja);
                  for (n = 0; n < nxgrid; n++)
                    {
                      ja = gdata_ja[n] - 1;
                  ...
                  if (na == tile_nest)
                    for (n = 0; n < nxgrid; n++)
                      (*axo_area_sum_nest) += gdata_dbl[n];
                  else
                    for (n = 0; n < nxgrid; n++)
                      (*axo_area_sum) += gdata_dbl[n];
                }
              mpp_put_var_value_block (fid, id_xgrid_area, start, nwrite,
                                       gdata_dbl);
              mpp_gather_field_int (naxo[na][no], atmxocn_ia[na][no],
                                    gdata_int);
              mpp_put_var_value_block (fid, id_tile1_cell, start, nwrite,
                                       gdata_int);
              mpp_gather_field_int (naxo[na][no], atmxocn_io[na][no],
                                    gdata_int);
              mpp_put_var_value_block (fid, id_tile2_cell, start, nwrite,
                                       gdata_int);
              start[1] = 1;
              ...
              mpp_put_var_value_block (fid, id_tile1_cell, start, nwrite,
                                       gdata_int);
              mpp_gather_field_int (naxo[na][no], atmxocn_jo[na][no],
                                    gdata_int);
              mpp_put_var_value_block (fid, id_tile2_cell, start, nwrite,
                                       gdata_int);
              if (interp_order == 2)
...
/* Helper: free atm<->(lnd,ocn) exchange arrays extracted from main */
static void free_atm_exchange_arrays(int ntile_atm, int ntile_lnd, int ntile_ocn,
                                     int interp_order,
                                     double ***atmxlnd_area,
                                     int ***atmxlnd_ia, int ***atmxlnd_ja,
                                     int ***atmxlnd_il, int ***atmxlnd_jl,
                                     ...
                                     int ***atmxocn_ia, int ***atmxocn_ja,
                                     int ***atmxocn_io, int ***atmxocn_jo,
                                     double ***atmxlnd_clon, double ***atmxlnd_clat,
                                     double ***atmxlnd_dia, double ***atmxlnd_dja,
                                     double ***atmxlnd_dil, double ***atmxlnd_djl,
                                     double ***atmxocn_clon, double ***atmxocn_clat,
                                     double ***atmxocn_dia, double ***atmxocn_dja,
                                     double ***atmxocn_dio, double ***atmxocn_djo,
                                     size_t **naxl, size_t **naxo)
{
...
/* Helper: allocate land->ocean exchange arrays extracted from main */
static void allocate_lndxocn_arrays(int ntile_lnd, int ntile_ocn, int interp_order,
                                    size_t ***nlxo_ptr,
                                    double ****lndxocn_area_ptr,
                                    int ****lndxocn_il_ptr, int ****lndxocn_jl_ptr,
                                    int ****lndxocn_io_ptr, int ****lndxocn_jo_ptr,
                                    double ****lndxocn_dil_ptr, double ****lndxocn_djl_ptr,
                                    double ****lndxocn_dio_ptr, double ****lndxocn_djo_ptr,
                                    double ****lndxocn_clon_ptr, double ****lndxocn_clat_ptr)
{
  int nl, no;

  *nlxo_ptr = (size_t **)malloc (ntile_lnd * sizeof (size_t *));
  *lndxocn_area_ptr = (double ***)malloc (ntile_lnd * sizeof (double **));
  *lndxocn_il_ptr = (int ***)malloc (ntile_lnd * sizeof (int **));
  *lndxocn_jl_ptr = (int ***)malloc (ntile_lnd * sizeof (int **));
  *lndxocn_io_ptr = (int ***)malloc (ntile_lnd * sizeof (int **));
  *lndxocn_jo_ptr = (int ***)malloc (ntile_lnd * sizeof (int **));

  if (interp_order == 2)
    {
      *lndxocn_dil_ptr = (double ***)malloc (ntile_lnd * sizeof (double **));
      *lndxocn_djl_ptr = (double ***)malloc (ntile_lnd * sizeof (double **));
      *lndxocn_dio_ptr = (double ***)malloc (ntile_lnd * sizeof (double **));
      *lndxocn_djo_ptr = (double ***)malloc (ntile_lnd * sizeof (double **));
      *lndxocn_clon_ptr = (double ***)malloc (ntile_lnd * sizeof (double **));
      *lndxocn_clat_ptr = (double ***)malloc (ntile_lnd * sizeof (double **));
    }

  for (nl = 0; nl < ntile_lnd; nl++)
    {
      (*nlxo_ptr)[nl] = (size_t *)malloc (ntile_ocn * sizeof (size_t));
      (*lndxocn_area_ptr)[nl] = (double **)malloc (ntile_ocn * sizeof (double *));
      (*lndxocn_il_ptr)[nl] = (int **)malloc (ntile_ocn * sizeof (int *));
      (*lndxocn_jl_ptr)[nl] = (int **)malloc (ntile_ocn * sizeof (int *));
      (*lndxocn_io_ptr)[nl] = (int **)malloc (ntile_ocn * sizeof (int *));
      (*lndxocn_jo_ptr)[nl] = (int **)malloc (ntile_ocn * sizeof (int *));

      if (interp_order == 2)
        {
          (*lndxocn_dil_ptr)[nl] = (double **)malloc (ntile_ocn * sizeof (double *));
          (*lndxocn_djl_ptr)[nl] = (double **)malloc (ntile_ocn * sizeof (double *));
          (*lndxocn_dio_ptr)[nl] = (double **)malloc (ntile_ocn * sizeof (double *));
          (*lndxocn_djo_ptr)[nl] = (double **)malloc (ntile_ocn * sizeof (double *));
          (*lndxocn_clon_ptr)[nl] = (double **)malloc (ntile_ocn * sizeof (double *));
          (*lndxocn_clat_ptr)[nl] = (double **)malloc (ntile_ocn * sizeof (double *));
        }

      for (no = 0; no < ntile_ocn; no++)
        {
          (*lndxocn_area_ptr)[nl][no] = (double *)malloc (get_maxxgrid() * sizeof (double));
          (*lndxocn_il_ptr)[nl][no] = (int *)malloc (get_maxxgrid() * sizeof (int));
          (*lndxocn_jl_ptr)[nl][no] = (int *)malloc (get_maxxgrid() * sizeof (int));
          (*lndxocn_io_ptr)[nl][no] = (int *)malloc (get_maxxgrid() * sizeof (int));
          (*lndxocn_jo_ptr)[nl][no] = (int *)malloc (get_maxxgrid() * sizeof (int));
          if (interp_order == 2)
            {
              (*lndxocn_dil_ptr)[nl][no] = (double *)malloc (get_maxxgrid() * sizeof (double));
              (*lndxocn_djl_ptr)[nl][no] = (double *)malloc (get_maxxgrid() * sizeof (double));
              (*lndxocn_dio_ptr)[nl][no] = (double *)malloc (get_maxxgrid() * sizeof (double));
              (*lndxocn_djo_ptr)[nl][no] = (double *)malloc (get_maxxgrid() * sizeof (double));
              (*lndxocn_clon_ptr)[nl][no] = (double *)malloc (get_maxxgrid() * sizeof (double *));
              (*lndxocn_clat_ptr)[nl][no] = (double *)malloc (get_maxxgrid() * sizeof (double *));
            }
        }
    }
}

/* Helper: compute land->ocean intersections and centroids extracted from main */
static void compute_lndxocn_and_centroids(int ntile_lnd, int ntile_ocn,
                                          int *nxl, int *nyl, int *nxo, int *nyo,
                                          size_t **nlxo,
                                          double ***lndxocn_area,
                                          int ***lndxocn_il, int ***lndxocn_jl,
                                          int ***lndxocn_io, int ***lndxocn_jo,
                                          double ***lndxocn_dil, double ***lndxocn_djl,
                                          double ***lndxocn_dio, double ***lndxocn_djo,
                                          double ***lndxocn_clon, double ***lndxocn_clat,
                                          int clip_method, int interp_order,
                                          double **xlnd, double **ylnd,
                                          double **cart_xlnd, double **cart_ylnd, double **cart_zlnd,
                                          double **xocn, double **yocn,
                                          double **cart_xocn, double **cart_yocn, double **cart_zocn,
                                          double **omask, double **area_ocn, double **area_lnd,
                                          double area_ratio_thresh)
{
  int nl, no, ll, i, lo;
  for (nl = 0; nl < ntile_lnd; nl++)
    {
      ...
      double xl[MV], yl[MV], zl[MV], xo[MV], yo[MV], zo[MV], x_out[MV],
          y_out[MV], z_out[MV];
      domain2D Dom;
      int *is_ocn, *ie_ocn;
      int *js_ocn, *je_ocn;
          ...
          for (no = 0; no < ntile_ocn; no++)
            {
              for (jo = js_ocn[no]; jo <= je_ocn[no]; jo++)
                for (io = is_ocn[no]; io <= ie_ocn[no]; io++)
                  if (omask[no][jo * nxo[no] + io] > MIN_AREA_FRAC)
                    {
                      double ocn_frac;
                      if (clip_method == GREAT_CIRCLE_CLIP)
                        { /* clockwise */
                      ...
                      else
                        {
                          ...
                          xo_min = minval_double (no_in, xo);
                          xo_max = maxval_double (no_in, xo);
                          /* xo should in the same range as xa after
                             lon_fix, so no need to consider cyclic
                             condition
                          */
                      ...
                      if (clip_method == GREAT_CIRCLE_CLIP)
                        {
                          n_out = clip_2dx2d_great_circle (
                              xl, yl, zl, 4, xo, yo, zo, 4, x_out, y_out,
                              z_out);
                        }
                      else
                        n_out = clip_2dx2d (xl, yl, nl_in, xo, yo, no_in,
                                            x_out, y_out);
                          ...
                          if (xarea / min_area > area_ratio_thresh)
                            {
                              ...
                              if (nlxo[nl][no] > get_maxxgrid())
                                {
                                  char msg[200];
                                  snprintf(msg, sizeof(msg), "nlxo (%d) exceeds MAXXGRID limit, increase the value using --maxxgrid flag", nlxo[nl][no]);
                                  mpp_error(msg);
                                }
                            }
                        }
      ...
      for (nl = 0; nl < ntile_lnd; nl++)
        {
          ...
          /* substract land centroid to get the centroid distance between
           * land grid and exchange grid. */
          for (no = 0; no < ntile_ocn; no++)
            {
              ...
              for (i = 0; i < nlxo[nl][no]; i++)
                {
                  ...
                  lndxocn_dio[nl][no][i]
                      = lndxocn_clon[nl][no][i] / lndxocn_area[nl][no][i] - o_clon[no][lo];
                  lndxocn_djo[nl][no][i]
                      = lndxocn_clat[nl][no][i] / lndxocn_area[nl][no][i] - o_clat[no][lo];
                }
            }
          free (o_area[no]);
          free (o_clon[no]);
...
/* Helper: write land->ocean exchange files extracted from main */
static void write_lndxocn_files(int ntile_lnd, int ntile_ocn,
                                size_t **nlxo,
                                double ***lndxocn_area,
                                int ***lndxocn_il, int ***lndxocn_jl,
                                int ***lndxocn_io, int ***lndxocn_jo,
                                ...
                                char *history, char *grid_version,
                                int clip_method, int interp_order,
                                int same_mosaic, int ocn_south_ext,
                                int *nfile_lxo)
{
  int nl, no, i, lo;
  for (nl = 0; nl < ntile_lnd; nl++)
          ...
          if (nxgrid > 0)
            {
              ...
              if (same_mosaic)
                sprintf (lxo_file[*nfile_lxo], "lnd_%s_%sXocn_%s_%s.nc",
                         lmosaic_name, ltile_name[nl], omosaic_name,
                         otile_name[no]);
              else
                sprintf (lxo_file[*nfile_lxo], "%s_%sX%s_%s.nc",
                         lmosaic_name, ltile_name[nl], omosaic_name,
                         otile_name[no]);
              sprintf (contact, "%s:%s::%s:%s", lmosaic_name,
              ...
              if (interp_order == 2)
                {
                  id_tile1_dist = mpp_def_var (
                      fid, "tile1_distance", MPP_DOUBLE, 2, dims, 1,
                      "standard_name",
                      "distance_from_parent1_cell_centroid");
                  id_tile2_dist = mpp_def_var (
                      fid, "tile2_distance", MPP_DOUBLE, 2, dims, 1,
                      "standard_name",
                      "distance_from_parent2_cell_centroid");
                }
              mpp_end_def (fid);

              /* adjust to Fortran indexing */
              for (i = 0; i < nlxo[nl][no]; i++)
                {
                  ++(lndxocn_il[nl][no][i]);
                  ++(lndxocn_jl[nl][no][i]);
                  ++(lndxocn_io[nl][no][i]);
                  lndxocn_jo[nl][no][i] += 1 - ocn_south_ext;
                }
...
/* Helper: free land->ocean exchange arrays extracted from main */
static void free_lndxocn_arrays(int ntile_lnd, int ntile_ocn, int interp_order,
                                double ***lndxocn_area,
                                int ***lndxocn_il, int ***lndxocn_jl,
                                int ***lndxocn_io, int ***lndxocn_jo,
                                double ***lndxocn_clon, double ***lndxocn_clat,
...
/* Helper: allocate wav->ocean exchange arrays extracted from main */
static void allocate_wavxocn_arrays(int ntile_wav, int ntile_ocn, int interp_order,
                                    size_t ***nwxo_ptr,
                                    double ****wavxocn_area_ptr,
                                    int ****wavxocn_iw_ptr, int ****wavxocn_jw_ptr,
                                    ...
                                    double ****wavxocn_clon_ptr, double ****wavxocn_clat_ptr)
{
  ...
  *nwxo_ptr = (size_t **)malloc (ntile_wav * sizeof (size_t *));
  *wavxocn_area_ptr = (double ***)malloc (ntile_wav * sizeof (double **));
  *wavxocn_iw_ptr = (int ***)malloc (ntile_wav * sizeof (int **));
  *wavxocn_jw_ptr = (int ***)malloc (ntile_wav * sizeof (int **));
  *wavxocn_io_ptr = (int ***)malloc (ntile_wav * sizeof (int **));
  *wavxocn_jo_ptr = (int ***)malloc (ntile_wav * sizeof (int **));

  if (interp_order == 2)
    {
      *wavxocn_diw_ptr = (double ***)malloc (ntile_wav * sizeof (double **));
      *wavxocn_djw_ptr = (double ***)malloc (ntile_wav * sizeof (double **));
      *wavxocn_dio_ptr = (double ***)malloc (ntile_wav * sizeof (double **));
      *wavxocn_djo_ptr = (double ***)malloc (ntile_wav * sizeof (double **));
      *wavxocn_clon_ptr = (double ***)malloc (ntile_wav * sizeof (double **));
      *wavxocn_clat_ptr = (double ***)malloc (ntile_wav * sizeof (double **));
    }

  for (nw = 0; nw < ntile_wav; nw++)
    {
      (*nwxo_ptr)[nw] = (size_t *)malloc (ntile_ocn * sizeof (size_t));
      (*wavxocn_area_ptr)[nw] = (double **)malloc (ntile_ocn * sizeof (double *));
      (*wavxocn_iw_ptr)[nw] = (int **)malloc (ntile_ocn * sizeof (int *));
      (*wavxocn_jw_ptr)[nw] = (int **)malloc (ntile_ocn * sizeof (int *));
      (*wavxocn_io_ptr)[nw] = (int **)malloc (ntile_ocn * sizeof (int *));
      (*wavxocn_jo_ptr)[nw] = (int **)malloc (ntile_ocn * sizeof (int *));

      if (interp_order == 2)
        {
          (*wavxocn_diw_ptr)[nw] = (double **)malloc (ntile_ocn * sizeof (double *));
          (*wavxocn_djw_ptr)[nw] = (double **)malloc (ntile_ocn * sizeof (double *));
          (*wavxocn_dio_ptr)[nw] = (double **)malloc (ntile_ocn * sizeof (double *));
          (*wavxocn_djo_ptr)[nw] = (double **)malloc (ntile_ocn * sizeof (double *));
          (*wavxocn_clon_ptr)[nw] = (double **)malloc (ntile_ocn * sizeof (double *));
          (*wavxocn_clat_ptr)[nw] = (double **)malloc (ntile_ocn * sizeof (double *));
        }

      for (no = 0; no < ntile_ocn; no++)
        {
          (*wavxocn_area_ptr)[nw][no] = (double *)malloc (get_maxxgrid() * sizeof (double));
          (*wavxocn_iw_ptr)[nw][no] = (int *)malloc (get_maxxgrid() * sizeof (int));
          (*wavxocn_jw_ptr)[nw][no] = (int *)malloc (get_maxxgrid() * sizeof (int));
          (*wavxocn_io_ptr)[nw][no] = (int *)malloc (get_maxxgrid() * sizeof (int));
          (*wavxocn_jo_ptr)[nw][no] = (int *)malloc (get_maxxgrid() * sizeof (int));
          if (interp_order == 2)
            {
              (*wavxocn_clon_ptr)[nw][no] = (double *)malloc (get_maxxgrid() * sizeof (double));
              (*wavxocn_clat_ptr)[nw][no] = (double *)malloc (get_maxxgrid() * sizeof (double));
              (*wavxocn_diw_ptr)[nw][no] = (double *)malloc (get_maxxgrid() * sizeof (double));
              (*wavxocn_djw_ptr)[nw][no] = (double *)malloc (get_maxxgrid() * sizeof (double));
              (*wavxocn_dio_ptr)[nw][no] = (double *)malloc (get_maxxgrid() * sizeof (double));
              (*wavxocn_djo_ptr)[nw][no] = (double *)malloc (get_maxxgrid() * sizeof (double));
            }
        }
    }
}

/* Helper: compute wave->ocean exchange grids (extracted from main) */
static void compute_wavxocn(int ntile_wav, int ntile_ocn,
                           int *nxw, int *nyw, int *nxo, int *nyo,
                           double **xwav, double **ywav,
                           double **cart_xwav, double **cart_ywav,
                           ...
                           double ***wavxocn_clon, double ***wavxocn_clat, unsigned int verbose)
{
      ...
      for (lw = is; lw <= ie; lw++)
        {
          ...
          if (clip_method == GREAT_CIRCLE_CLIP)
            { /* clockwise */
              ...
              n2 = (jw + 1) * (nxw[nw] + 1) + iw + 1;
              n3 = jw * (nxw[nw] + 1) + iw + 1;
              xw[0] = cart_xwav[nw][n0];
              yw[0] = cart_ywav[nw][n0];
              zw[0] = cart_zwav[nw][n0];
              xw[1] = cart_xwav[nw][n1];
              yw[1] = cart_ywav[nw][n1];
              zw[1] = cart_zwav[nw][n1];
              xw[2] = cart_xwav[nw][n2];
              yw[2] = cart_ywav[nw][n2];
              zw[2] = cart_zwav[nw][n2];
              xw[3] = cart_xwav[nw][n3];
              yw[3] = cart_ywav[nw][n3];
              zw[3] = cart_zwav[nw][n3];
            }
          else
            {
              n0 = jw * (nxw[nw] + 1) + iw;
                    ...
                    if (ocn_frac > MIN_AREA_FRAC)
                      { /* over sea/ice */
                        if (clip_method == GREAT_CIRCLE_CLIP)
                          {
                            n_out = clip_2dx2d_great_circle (
                                xw, yw, zw, 4, xo, yo, zo, 4, x_out, y_out,
                                z_out);
                          }
                        else
                          {
                            if (xw_min >= xo_max || xw_max <= xo_min
                                || yw_min >= yo_max || yw_max <= yo_min)
                              continue;
                            n_out = clip_2dx2d (xw, yw, nw_in, xo, yo,
                                                no_in, x_out, y_out);
                          }
                        if (n_out > 0)
                          {
                            ...
                            if (xarea / min_area > area_ratio_thresh)
                              {
                                ...
                                if (nwxo[nw][no] > get_maxxgrid())
                                  {
                                    char msg[200];
                                    snprintf(msg, sizeof(msg), "nwxo (%d) exceeds MAXXGRID limit, increase the value using --maxxgrid flag", nwxo[nw][no]);
                                    mpp_error(msg);
                                  }
                              }
                          }
  ...
  time_end = time (NULL);
  if (verbose)
    printf ("on pe %d, calculating waveXocean used %f seconds.\n",
            mpp_pe (), difftime (time_end, time_start));
}

/* Helper: finalize wavXocn centroids, compute masks and write wave mask files */
static void write_wav_masks_and_centroids(int ntile_wav, int ntile_ocn,
                                         int *nxw, int *nyw,
                                         int *nxo, int *nyo,
                                         size_t **nwxo,
...
{
  {
    ...
    for (nw = 0; nw < ntile_wav; nw++)
      {
        w_area[nw]
            = (double *)malloc (nxw[nw] * nyw[nw] * sizeof (double));
    ...
    if (interp_order == 1)
                ...
                if (nxgrid > 0)
                  {
                    ...
                    g_area = (double *)malloc (nxgrid * sizeof (double));
                    mpp_gather_field_int (nwxo[nw][no], wavxocn_io[nw][no],
                                          g_iw);
                    mpp_gather_field_int (nwxo[nw][no], wavxocn_jo[nw][no],
                                          g_jw);
                    mpp_gather_field_double (nwxo[nw][no],
                                             wavxocn_area[nw][no], g_area);
      ...
      { /* interp_order == 2 */
        ...
        for (int no = 0; no < ntile_ocn; no++)
          {
            o_area[no]
                = (double *)malloc (nxo[no] * nyo[no] * sizeof (double));
            o_clon[no]
                = (double *)malloc (nxo[no] * nyo[no] * sizeof (double));
            o_clat[no]
                = (double *)malloc (nxo[no] * nyo[no] * sizeof (double));
        ...
        for (nw = 0; nw < ntile_wav; nw++)
          {
                ...
                if (nxgrid > 0)
                  {
                    ...
                    g_clon = (double *)malloc (nxgrid * sizeof (double));
                    g_clat = (double *)malloc (nxgrid * sizeof (double));
                    mpp_gather_field_int (nwxo[nw][no], wavxocn_iw[nw][no],
                                          g_iw);
                    mpp_gather_field_int (nwxo[nw][no], wavxocn_jw[nw][no],
                                          g_jw);
                    mpp_gather_field_int (nwxo[nw][no], wavxocn_io[nw][no],
                                          g_io);
                    mpp_gather_field_int (nwxo[nw][no], wavxocn_jo[nw][no],
                    ...
                    mpp_gather_field_double (nwxo[nw][no],
                                             wavxocn_area[nw][no], g_area);
                    mpp_gather_field_double (nwxo[nw][no],
                                             wavxocn_clon[nw][no], g_clon);
                    mpp_gather_field_double (nwxo[nw][no],
                                             wavxocn_clat[nw][no], g_clat);
                    for (int i = 0; i < nxgrid; i++)
                      {
                        ii = g_jw[i] * nxw[nw] + g_iw[i];
            ...
            for (int no = 0; no < ntile_ocn; no++)
              {
                for (int i = 0; i < nwxo[nw][no]; i++)
                  {
                    lw = wavxocn_jw[nw][no][i] * nxw[nw]
                         + wavxocn_iw[nw][no][i];
                    wavxocn_diw[nw][no][i]
                        = wavxocn_clon[nw][no][i] / wavxocn_area[nw][no][i]
                          - w_clon[lw];
                    wavxocn_djw[nw][no][i]
                        = wavxocn_clat[nw][no][i] / wavxocn_area[nw][no][i]
                          - w_clat[lw];
                  }
              }
      ...
      for (nw = 0; nw < ntile_wav; nw++)
        {
          ...
          for (jw = 0; jw < nyw[nw]; jw++)
            for (iw = 0; iw < nxw[nw]; iw++)
              {
                int i = jw * nxw[nw] + iw;
...
/* Helper: write wav->ocean exchange files (extracted from main) */
static void write_wavxocn_files(int ntile_wav, int ntile_ocn,
                               size_t **nwxo,
                               double ***wavxocn_area,
                               int ***wavxocn_iw, int ***wavxocn_jw,
          ...
          if (nxgrid > 0)
            {
              ...
              if (same_mosaic)
                sprintf (wxo_file[*nfile_wxo], "wav_%s_%sXocn_%s_%s.nc",
                         wmosaic_name, wtile_name[nw], omosaic_name,
                         otile_name[no]);
              else
                sprintf (wxo_file[*nfile_wxo], "%s_%sX%s_%s.nc",
                         wmosaic_name, wtile_name[nw], omosaic_name,
                         otile_name[no]);
              sprintf (contact, "%s:%s::%s:%s", wmosaic_name,
                       wtile_name[nw], omosaic_name, otile_name[no]);
              ...
              for (i = 0; i < nwxo[nw][no]; i++)
                {
                  ...
                  ++(wavxocn_io[nw][no][i]);
                  wavxocn_jo[nw][no][i]
                      += 1 - ocn_south_ext; /* one artificial j-level may
                                               be added in the south end */
                }
              ...
              mpp_gather_field_double (nwxo[nw][no], wavxocn_area[nw][no],
                                       gdata_dbl);
              if (check)
                {
                  for (n = 0; n < nxgrid; n++)
                    (*wxo_area_sum) += gdata_dbl[n];
                }
              mpp_put_var_value (fid, id_xgrid_area, gdata_dbl);
              mpp_gather_field_int (nwxo[nw][no], wavxocn_iw[nw][no],
                                    gdata_int);
              ...
              if (interp_order == 2)
                {
                  start[1] = 0;
                  mpp_gather_field_double (nwxo[nw][no],
                                           wavxocn_diw[nw][no], gdata_dbl);
                  mpp_put_var_value_block (fid, id_tile1_dist, start,
                                           nwrite, gdata_dbl);
                  mpp_gather_field_double (nwxo[nw][no],
                                           wavxocn_dio[nw][no], gdata_dbl);
                  mpp_put_var_value_block (fid, id_tile2_dist, start,
                                           nwrite, gdata_dbl);
                  start[1] = 1;
                  mpp_gather_field_double (nwxo[nw][no],
                                           wavxocn_djw[nw][no], gdata_dbl);
                  mpp_put_var_value_block (fid, id_tile1_dist, start,
                                           nwrite, gdata_dbl);
                  mpp_gather_field_double (nwxo[nw][no],
                                           wavxocn_djo[nw][no], gdata_dbl);
                  mpp_put_var_value_block (fid, id_tile2_dist, start,
                                           nwrite, gdata_dbl);
                }
              mpp_close (fid);
...
/* Helper: free wav->ocean exchange arrays (extracted from main) */
static void free_wavxocn_arrays(int ntile_wav, int ntile_ocn, int interp_order,
                               size_t **nwxo,
                               double ***wavxocn_area,
                               int ***wavxocn_iw, int ***wavxocn_jw,
      ...
      if (check)
        {
          ...
          for (n = 0; n < ntile_atm; n++)
            {
              for (i = 0; i < nxa[n] * nya[n]; i++)
                {
                  cur_ratio = fabs (atm_xarea[n][i] - area_atm[n][i])
                              / area_atm[n][i];
                  if (cur_ratio > 1.e-5)
                    {
                      printf ("at tile =%d, i=%d, j=%d, ratio=%g, area1=%g, "
                              "area2=%g\n",
                              n + 1, i % nxa[n], i / nxa[n], cur_ratio,
                              area_atm[n][i], atm_xarea[n][i]);
                    }
          ...
          printf ("The maximum area mismatch is at tile=%d, i=%d, j=%d, "
                  "ratio=%g, area_atm_grid=%g, area_atm_xgrid=%g\n",
                  max_ta + 1, max_ia, max_ja, max_ratio,
                  area_atm[max_ta][max_ja * nxa[max_ta] + max_ia],
          ...
          for (n = 0; n < ntile_atm; n++)
            {
              if (n == tile_nest)
                for (i = 0; i < nxa[n] * nya[n]; i++)
                  atm_area_sum_nest += area_atm[n][i];
              else
                for (i = 0; i < nxa[n] * nya[n]; i++)
                  atm_area_sum += area_atm[n][i];
            }
          ...
          tiling_area = (atm_area_sum - axo_area_sum - axl_area_sum)
                        / atm_area_sum * 100;
          ...
          if (tile_nest >= 0)
            {
              ...
              tiling_area_nest
                  = (atm_area_sum_nest - axo_area_sum_nest - axl_area_sum_nest)
                    / atm_area_sum_nest * 100;
              printf ("\nNOTE: axo_area_sum_nest is %f and ocean fraction is "
                      "%f%%\n",
                      axo_area_sum_nest, axo_area_frac_nest);
              printf (
                  "NOTE: axl_area_sum_nest is %f and land  fraction is %f%%\n",
...
int main (int argc, char *argv[])
{
  ...
  */
  ...
  if (wmosaic)
    get_file_dir_and_name (wmosaic, wmosaic_dir, wmosaic_file);
  get_file_dir_and_name (otopog, otopog_dir, otopog_file);
  if (!strcmp (mosaic_file, amosaic_file)
      || !strcmp (mosaic_file, lmosaic_file)
      || !strcmp (mosaic_file, omosaic_file))
    mpp_error ("make_coupler_mosaic: mosaic_file can not have the same name "
               "as amosaic, lmosaic or omosaic");
  if (!strcmp (amosaic_file, "mosaic.nc")
      || !strcmp (lmosaic_file, "mosaic.nc")
      || !strcmp (omosaic_file, "mosaic.nc"))
    mpp_error ("make_coupler_mosaic: the file name of amosaic, lmosaic or "
               "omosaic can not be mosaic.nc");

  if (print_memory)
    print_mem_usage ("before read atmosphere grid");

  /*
   * Read atmosphere grid
  */
  read_atmos_mosaic_block(amosaic, amosaic_name, x_refine, y_refine, interp_order, check,
                          &ntile_atm, &nxa, &nya, &xatm, &yatm, &area_atm,
                          &atm_xarea, &atile_name, &atm_great_circle_algorithm,
                          &clip_method, &cart_xatm, &cart_yatm, &cart_zatm,
                          &is_nest, &ie_nest, &js_nest, &je_nest,
                          &is_parent, &ie_parent, &js_parent, &je_parent,
                          &tile_nest, &tile_parent);
  if (print_memory)
    print_mem_usage ("after read atmosphere grid");
  /*
   * Read land grid
   */
  if (strcmp (lmosaic, amosaic))
    {
      read_land_mosaic_block(lmosaic, lmosaic_name, x_refine, y_refine, clip_method,
                             &ntile_lnd, &nxl, &nyl, &xlnd, &ylnd, &area_lnd,
                             &ltile_name, &lnd_great_circle_algorithm,
                             &cart_xlnd, &cart_ylnd, &cart_zlnd);
    }
  else
    { /* land mosaic is same as atmosphere mosaic */
      if (tile_nest >= 0)
        mpp_error ("make_coupler_mosaic: land mosaic must be different from "
                   "atmos mosaic "
  ...
  for (n = 0; n < ntile_lnd; n++)
    {
      if (mpp_pe () == mpp_root_pe () && verbose)
        printf ("Number of ATM and LND cells for tile %d are %d, %d.\n", n + 1,
                nxa[n] * nya[n], nxl[n] * nyl[n]);
      if (nxa[n] * nya[n] != nxl[n] * nyl[n])
        printf ("Warning: Number of ATM and LND cells for tile %d are not "
                "equal %d, %d.\n",
                n + 1, nxa[n] * nya[n], nxl[n] * nyl[n]);
    }
  if (print_memory)
    print_mem_usage ("after read land grid");

  if (strcmp (omosaic, amosaic) == 0)
    ocn_same_as_atm = 1;
  /*
   * Read ocean grid boundaries and mask (where water is) for each tile within
   * the mosaic.
   */
  read_ocean_mosaic_block(omosaic, omosaic_name, x_refine, y_refine,
                          &ntile_ocn, &nxo, &nyo, &xocn, &yocn, &area_ocn,
                          &otile_name, &ocn_south_ext,
                          &ocn_great_circle_algorithm, clip_method,
                          &cart_xocn, &cart_yocn, &cart_zocn,
                          otopog, &omask, sea_level, verbose);
  if (print_memory)
    print_mem_usage ("after read ocean grid");

  /* when atm_great_circle_algorithm is 0,
   * lnd_great_circle_algorithm/ocn_great_circle_algorithm must be 0 */
  if (atm_great_circle_algorithm == 0
      && (lnd_great_circle_algorithm || ocn_great_circle_algorithm))
    mpp_error (
        "make_coupler_mosaic: when atm does not use great_circle_algorithm, "
        "lnd/ocn can not use great_circle_algorithm");

  /*
   * Read wave grid
   */
  if (wmosaic)
    read_wave_mosaic_block(wmosaic, omosaic, wmosaic_name, x_refine, y_refine,
                           &ntile_wav, &nxw, &nyw, &xwav, &ywav, &area_wav,
                           &wtile_name, &wav_same_as_ocn, clip_method,
                           &cart_xwav, &cart_ywav, &cart_zwav);
  ...
  */
  if (strcmp (omosaic_name, amosaic_name))
    { /* omosaic is different from amosaic */
  ...
  ***************************************************************************************/
  ...
  allocate_atm_exchange_arrays(ntile_atm, ntile_lnd, ntile_ocn, interp_order,
                                &naxl, &naxo,
                                &atmxlnd_area,
                                &atmxlnd_ia, &atmxlnd_ja,
                                &atmxlnd_il, &atmxlnd_jl,
                                &atmxocn_area,
                                &atmxocn_ia, &atmxocn_ja,
                                &atmxocn_io, &atmxocn_jo,
                                &atmxlnd_dia, &atmxlnd_dja,
                                &atmxlnd_dil, &atmxlnd_djl,
  ...
  /* compute the atm-lnd-ocn exchange grids */
  compute_atm_lnd_ocn_exchange(ntile_atm, ntile_lnd, ntile_ocn,
                                nxa, nya, xatm, yatm, area_atm, atm_xarea,
                                nxl, nyl, xlnd, ylnd, area_lnd,
                                nxo, nyo, xocn, yocn, area_ocn,
                                omask, interp_order, clip_method,
                                area_ratio_thresh, verbose,
                                lnd_same_as_atm,ocn_same_as_atm,
                                print_memory, naxl, naxo, atmxlnd_ia,
                                atmxlnd_ja, atmxlnd_il, atmxlnd_jl,
                                atmxocn_ia, atmxocn_ja, atmxocn_io,
                                atmxocn_jo, atmxlnd_area, atmxocn_area,
                                atmxlnd_clon, atmxlnd_clat,
                                atmxocn_clon, atmxocn_clat,
                                cart_xatm, cart_yatm, cart_zatm,
                                cart_xlnd, cart_ylnd, cart_zlnd,
                                cart_xocn, cart_yocn, cart_zocn);
  ...
  time_end = time (NULL);
  if (verbose)
    printf ("one pe %d, The loop used %f seconds.\n", mpp_pe (),
            difftime (time_end, time_start));
  /* calculate the centroid of model grid, as well as land_mask and
    * ocean_mask */
  finalize_and_write_masks(ntile_atm, ntile_lnd, ntile_ocn,
                            nxa, nya, nxl, nyl,
                            nxo, nyo, naxl,
                            naxo,
                            atmxlnd_ia, atmxlnd_ja,
                            ...
                            area_lnd, area_ocn,
                            area_atm, omask,
                            ocn_south_ext, interp_order,
                            tile_nest, history,
                            grid_version, clip_method,
                            verbose, &nbad);

  write_atm_exchange_files(ntile_atm, ntile_lnd, ntile_ocn,
                            tile_nest, nxa, nya, nxl, nyl,
                            nxo, nyo,
                            naxl, naxo,
                            atmxlnd_ia, atmxlnd_ja,
                            atmxlnd_il, atmxlnd_jl,
                            ...
                            atm_xarea,
                            &nfile_axl, &nfile_axo,
                            &axl_area_sum, &axl_area_sum_nest,
                            &axo_area_sum, &axo_area_sum_nest,
                            ocn_south_ext);

  /* release atm<->(lnd,ocn) exchange arrays */
  free_atm_exchange_arrays(ntile_atm, ntile_lnd, ntile_ocn, interp_order,
                            atmxlnd_area,
                            atmxlnd_ia, atmxlnd_ja,
                            atmxlnd_il, atmxlnd_jl,
                            atmxocn_area,
  ...
  if (!lnd_same_as_atm)
    {
      ...
      if (mpp_pe () == mpp_root_pe () && verbose)
        printf ("\nNOTE from make_coupler_mosaic: lmosaic is different from  amosaic, "
                "we need to compute the exchange grid between lmosaic and omosaic.\n");

      /* allocate land->ocean exchange arrays */
      allocate_lndxocn_arrays(ntile_lnd, ntile_ocn, interp_order,
                               &nlxo,
                               &lndxocn_area,
                               &lndxocn_il, &lndxocn_jl,
                               &lndxocn_io, &lndxocn_jo,
                               &lndxocn_dil, &lndxocn_djl,
                               &lndxocn_dio, &lndxocn_djo,
                               &lndxocn_clon, &lndxocn_clat);

      /* compute land->ocean intersection and centroids */
      compute_lndxocn_and_centroids(ntile_lnd, ntile_ocn,
                                    nxl, nyl, nxo, nyo,
                                    nlxo,
                                    lndxocn_area,
                                    lndxocn_il, lndxocn_jl,
                                    ...
                                    xocn, yocn,
                                    cart_xocn, cart_yocn, cart_zocn,
                                    omask, area_ocn, area_lnd,
                                    area_ratio_thresh);

      /* write out lndXocn data */
      write_lndxocn_files(ntile_lnd, ntile_ocn,
                          nlxo,
                          lndxocn_area,
                          lndxocn_il, lndxocn_jl,
                          lndxocn_io, lndxocn_jo,
                          ...
                          history, grid_version,
                          clip_method, interp_order,
                          same_mosaic, ocn_south_ext,
                          &nfile_lxo);

      /* release the memory */
      /* release land->ocean exchange arrays */
      free_lndxocn_arrays(ntile_lnd, ntile_ocn, interp_order,
                          lndxocn_area,
                          lndxocn_il, lndxocn_jl,
                          lndxocn_io, lndxocn_jo,
                          lndxocn_clon, lndxocn_clat,
  ...
  else
    {
      ...
      if (mpp_pe () == mpp_root_pe () && verbose)
        printf ("\nNOTE from make_coupler_mosaic: Since lmosaic is the same "
                "as amosaic, "
                "no need to compute the exchange grid between lmosaic and "
                "omosaic, "
  ...
  ***************************************************************************************/
  ...
  write_mosaic_file(mosaic_file, history, grid_version, clip_method,
                    ...
                    amosaic_dir, amosaic_file, amosaic_name,
                    lmosaic_dir, lmosaic_file, lmosaic_name,
  ...
  if (mpp_pe () == mpp_root_pe () && verbose)
    {
      if (nbad == 0)
        ...
        {
          printf ("\n***** You have run make_coupler_mosaic, but there were "
                  "at least %d grid cells with issues.\n",
                  nbad);
